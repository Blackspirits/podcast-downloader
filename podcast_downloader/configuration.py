...
CONFIG_PODCASTS_REQUIRE_DATE = "require_date" # Depreciated, but kept constant
...
def get_n_age_date(day_number: int, from_date: time.struct_time) -> time.struct_time:
    # Converts time.struct_time to datetime to perform the calculations
    from_datetime = datetime(*from_date[:6])
    
    # Converte time.struct_time em datetime para efetuar os cálculos
    result_datetime = from_datetime - timedelta(days=day_number)
